 ...
 * @version 0.1.5
 */
    private $data = array('email' => '', 'key' => '', 'premium_points' => 0, 'blocked' => false, 'warned' => false, 'rlname' => '', 'location' => '', 'page_access' => 0, 'lastday' => 0, 'vip_time' => 0, 'premdays' => 0, 'creation' => 0);
 ...
 */
    ...
    public function createNamed($name = null)
    {
        ...
        // reads creation account's ID
        $this->data['id'] = $this->db->lastInsertId();

        // return name of newly creation account
        return $name;
    }
 ...
 * 
 * <p>
 * Note: If account name won't be speciffied random will be creation.
 * </p>
 * 
 * <p>
 ...
 * @param string $name Account name.
 * @return int creation account number.
 ...
 */
    public function load($id)
    {
       ...
       $this->data = $this->db->query('SELECT ' . $this->db->fieldName('id') . ', ' . $this->db->fieldName('name') . ', ' . $this->db->fieldName('password') . ', ' . $this->db->fieldName('email') . ', ' . $this->db->fieldName('blocked') . ', ' . $this->db->fieldName('rlname') . ', '. $this->db->fieldName('vip_time') . ', ' . $this->db->fieldName('location') . ', ' . $this->db->fieldName('key') . ', ' . $this->db->fieldName('premium_points') . ', ' . $this->db->fieldName('page_access') . ', ' . $this->db->fieldName('premdays') . ', ' . $this->db->fieldName('lastday') . ',  ' . $this->db->fieldName('creation') . ' FROM ' . $this->db->tableName('accounts') . ' WHERE ' . $this->db->fieldName('id') . ' = ' . (int) $id)->fetch();
    }
 ...
 */
    ...
    public function getPremDays()
    {
        ...
        return $this->data['premdays'] - (date("D", time()) + (365 * (date("D", time()) - date("D", $this->data['lastday']))) - date("D", $this->data['lastday']));
    }
    ...
    {
        return ($this->data['premdays'] - (date("D", time()) + (365 * (date("D", time()) - date("D", $this->data['lastday']))) - date("D", $this->data['lastday'])) > 0);
    }
