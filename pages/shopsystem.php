...
if($config['site']['shop_system'])
{
		...
		while($data = $offer_list->fetch())
		{
			...
			elseif($data['offer_type'] == 'mounts')
			{
                         ...
                         elseif($data['offer_type'] == 'addons')
			 {
				...
				$offer_array['addons'][$i_addons]['mounts_id'] = $data['itemid1'];
				$offer_array['addons'][$i_addons]['mounts_count'] = $data['count1'];
	...
	if(($action == '') or ($action == 'item') or ($action == 'mounts') or ($action == 'addons'))
	{
		...
		if((count($offer_list['item']) > 0) or (count($offer_list['mounts']) > 0) or (count($offer_list['addons']) > 0))
		{
			...
			if(count($offer_list['item']) > 0) $main_content .= '<a href="?subtopic=shopsystem&action=item" style="padding: 7px 1px 1px 8px; margin: 1px 1px 1px 195px; background-color: '.selectcolor('item').';">Items<img src="images/items/2160.gif"/></a>';
			if(count($offer_list['mounts']) > 0) $main_content .= '<a href="?subtopic=shopsystem&action=mounts" style="padding: 7px 1px 1px 20px; margin: 1px 1px 1px 1px; background-color: '.selectcolor('mounts').';">Mounts<img src="images/mount.gif"/></a>';
                        if(count($offer_list['addons']) > 0) $main_content .= '<a href="?subtopic=shopsystem&action=addons" style="padding: 7px 1px 1px 20px; margin: 1px 1px 1px 1px; background-color: '.selectcolor('addons').';">Addons<img src="images/addon.gif"/></a>';
		...
		if((count($offer_list['mounts']) > 0) and ($action == 'mounts'))
		{
			...
			{
				if(!is_int($number_of_rows / 2)) { $bgcolor = $config['site']['darkborder']; } else { $bgcolor = $config['site']['lightborder']; } $number_of_rows++;
				$main_content .= '<tr bgcolor="'.$bgcolor.'"><td align="center"><b>'.$mounts['points'].'</b></td><td align="center"><img src="' . $config['site']['item_imagems_url'] . $mounts['item_id'] . $config['site']['mounts_images_extension'] . '"></td><td><b>'.htmlspecialchars($mounts['name']).'</b> ('.$mounts['points'].' points)<br />'.htmlspecialchars($item['description']).'</td><td align="center">';
                                if(!$logged)
				{
					$main_content .= '<b>Login to buy</b>';
				}
				...
				{
					$main_content .= '<form action="?subtopic=shopsystem&action=select_player" method="POST" name="itemform_'.$mount['id'].'"><input type="hidden" name="buy_id" value="'.$mounts['id'].'"><div class="navibutton"><a href="" onClick="itemform_'.$mounts['id'].'.submit();return false;">BUY</a></div></form>';
				}
				$main_content .= '</td></tr>';
			}
			$main_content .= '</table>';
		...
		if((count($offer_list['addons']) > 0) and ($action == 'addons'))
		{
			...
			{
				if(!is_int($number_of_rows / 2)) { $bgcolor = $config['site']['darkborder']; } else { $bgcolor = $config['site']['lightborder']; } $number_of_rows++;
				$main_content .= '<tr bgcolor="'.$bgcolor.'"><td align="center"><b>'.$addons['points'].'</b></td><td align="center"><img src="' . $config['site']['item_images_url'] . $addons['item_id'] . $config['site']['item_images_extension'] . '"></td><td><b>'.htmlspecialchars($addons['name']).'</b> ('.$item['points'].' points)<br />'.htmlspecialchars($addons['description']).'</td><td align="center">';
                                if(!$logged)
				{
					$main_content .= '<b>Login to buy</b>';
				}
				...
				{
					$main_content .= '<form action="?subtopic=shopsystem&action=select_player" method="POST" name="itemform_'.$addons['id'].'"><input type="hidden" name="buy_id" value="'.$addons['id'].'"><div class="navibutton"><a href="" onClick="itemform_'.$item['id'].'.submit();return false;">BUY</a></div></form>';
				}
				$main_content .= '</td></tr>';
			}
			$main_content .= '</table>';
