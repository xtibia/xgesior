        ...
        if(!empty($_REQUEST['id']) and $bug[2] != NULL)
        {
            ...
            if(!$reply)
            {
                ...
                foreach($answers as $answer)
                {
                    ...
                    $main_content .= '<TR BGCOLOR="'.$dark.'"><td width=70%><img src=images/bug/tibia.png><i><b> Enviado por:</b></i></td><td>'.$who.'</td></tr>';
                    $main_content .= '<TR BGCOLOR="'.$light.'"><td colspan=2><img src=images/bug/des.png><i><b>Descrição:</b></i></td></tr>';
                    $main_content .= '<TR BGCOLOR="'.$light.'"><td colspan=2>'.nl2br($answer['text']).'</td></tr>';    
                    $main_content .= '</TABLE>';
                }
                if($bug[2]['status'] != 3)
...
//
    ...
    {
        $main_content .= ' - <a href="index.php?subtopic=helpdesk&control=true">[ADMIN PANEL]</a>';
    }
}
else
{
